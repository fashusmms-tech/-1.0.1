...
DEFAULT_SETTINGS = {
    ...
    "cut": {                    # 切割费用(分档)
        ...
        "band3": {"min_t": 3.0, "flat_w": 600.0, "flat_fee": 1.0,           # ≥3: w≤600→1, w>600→w/1000+0.6
                  "formula_extra": 0.6},
        # 注: 各档 min_t 仅作参考(用于说明分档区间), 计算只按 max_t 边界判定, 未在设置界面开放修改
    },
    ...
    "punch_hole": {                                     # 冲孔(选配件)
        ...
        "auto_flat_fee": 0.5,
        "manual_min_t": 1.5,     # 仅作参考(板厚≥此值建议手动), 计算只按 auto_max_t 判定
    },
    "accessory": {
...
def eval_expr(expr, env):
    ...
    def walk(node):
            ...
            if type(node.op) is ast.Pow:
                ...
                if isinstance(right, (int, float)):
                    if isinstance(right, float) and not math.isfinite(right):
                        raise ValueError("公式中的指数无效")
                    if abs(right) > 1024:
                        raise ValueError("公式中的指数过大")
                return _op.pow(left, right)
        ...
        if (isinstance(node, ast.Call) and isinstance(node.func, ast.Name)
            ...
            if len(node.args) == 2:
                nd = walk(node.args[1])
                if not isinstance(nd, (int, float)) or (isinstance(nd, float) and not math.isfinite(nd)):
                    raise ValueError("round 位数无效")
                nd = int(nd)
            else:
                nd = 0
            return round(v, nd)
    ...
    if isinstance(result, complex):
        raise ValueError("公式结果不能是复数(如负数开方)")
    if not isinstance(result, (int, float)):
        raise ValueError(f"公式结果类型异常: {type(result).__name__}")
    if isinstance(result, float) and not math.isfinite(result):
        raise ValueError("公式结果不是有限数值(如除零/溢出)")
    return result
...
def load_settings(path=None):
    s, _warns = _load_settings(path)
    return s


def load_settings_with_warning(path=None):
    """返回 (settings, [警告字符串]) — GUI 可在配置损坏时提示用户"""
...
def _load_settings(path):
    ...
    if path and os.path.exists(path):
        try:
            ...
            warns.append(f"settings.json 无法读取({e})，已备份为 .bak 并恢复默认设置")
            try:
...
def plate_price_full(thickness, width, pin_d, pitch_val, sheet_price, s, fees=None):
    """板价格(元/片) — 公式可在设置中修改
    fees=(cut, punch, weld) 可选, 由调用方算好后传入, 避免重复计算"""
    if fees is None:
        fees = (cut_fee(thickness, width, s),
                punch_fee(thickness, width, s),
                weld_fee(pin_d, s))
    cut, punch_f, weld = fees
    c = s["const"]
    env = {"pin_d": pin_d, "pitch": pitch_val, "thickness": thickness,
           "width": width, "sheet_price": sheet_price,
           "density": c["density"], "pin_pi": c["pin_pi"],
           "cut_fee": cut, "punch_fee": punch_f, "weld_fee": weld}
...
def format_result(r):
    ...
    L = []
    L.append("=" * 46)
    pitch_label = r["pitch_key"]
    if pitch_label == "假两寸":
        pitch_label = f"{r['pitch_val']:g} ({pitch_label})"
    L.append(f"金属链板成本明细  节距 {pitch_label} mm")
    L.append("=" * 46)
...
def result_csv_rows(r):
    """把明细 dict 转成 CSV 行 (每行 4 列), 供导出功能使用"""
    pm = r["per_meter"]
    rows = [
        ["类别", "项目", "数值(元)", "单位"],
        ["每片/每件", "板价格(材料+切割+冲床+焊接)", f"{r['plate']['total']:.2f}", "元/片"],
        ["每片/每件", "  板-材料", f"{r['plate']['material']:.2f}", "元/片"],
        ["每片/每件", "  板-切割", f"{r['plate']['cut']:.2f}", "元/片"],
        ["每片/每件", "  板-冲床", f"{r['plate']['punch']:.2f}", "元/片"],
        ["每片/每件", "  板-焊接", f"{r['plate']['weld']:.2f}", "元/片"],
        ["每片/每件", "穿杆价格", f"{r['pin']['total']:.2f}", "元/根"],
        ["每片/每件", "链条价格", f"{r['chain']['unit']:.2f}", "元/米"],
    ]
    if r["side"]:
        rows.append(["每片/每件", "侧挡板价格", f"{r['side']['price']:.2f}", "元/件"])
    if r["cross"]:
        rows.append(["每片/每件", "横挡板价格", f"{r['cross']['price']:.2f}", "元/件"])
        rows.append(["每片/每件", "横挡板数量", f"{r['cross']['count']}", "件"])
    if r["punch"]:
        rows.append(["每片/每件", "冲孔价格", f"{r['punch']['price']:.2f}", "元/片"])
    if r["ball"]:
        rows.append(["每片/每件", "辅助载重滚珠", f"{r['ball']['unit']:.2f}", "元/个"])
    if r["wheel"]:
        rows.append(["每片/每件", "辅助载重支轮", f"{r['wheel']['unit']:.2f}", "元/个"])

    rows.append(["每米总价", "板", f"{r['plate']['total'] * pm:.2f}", "元/米"])
    rows.append(["每米总价", "穿杆", f"{r['pin']['total'] * pm:.2f}", "元/米"])
    rows.append(["每米总价", "链条(×2)", f"{r['chain']['total']:.2f}", "元/米"])
    if r["side"]:
        rows.append(["每米总价", "侧挡板", f"{r['side']['total']:.2f}", "元/米"])
    if r["cross"]:
        rows.append(["每米总价", "横挡板", f"{r['cross']['total']:.2f}", "元/米"])
    if r["punch"]:
        rows.append(["每米总价", "冲孔", f"{r['punch']['price'] * pm:.2f}", "元/米"])
    if r["ball"]:
        rows.append(["每米总价", "滚珠", f"{r['ball']['total']:.2f}", "元/米"])
    if r["wheel"]:
        rows.append(["每米总价", "支轮", f"{r['wheel']['total']:.2f}", "元/米"])
    if r["labor"]:
        rows.append(["每米总价", "人工", f"{r['labor'] * pm:.2f}", "元/米"])
    rows.append(["每米总价", "合计", f"{r['total_per_meter']:.2f}", "元/米"])
    return rows
